...
class NewsEmbedder(nn.Module):
    ...
    def __load_glove_vectors(self, filename = "glove.42B.300d.txt") -> Tuple[List[str], torch.Tensor]:
        """Load the GloVe vectors. See: `https://github.com/stanfordnlp/GloVe`"""
        path = Path(hf_hub_download(repo_id="stanfordnlp/glove", filename="glove.42B.300d.zip"))
